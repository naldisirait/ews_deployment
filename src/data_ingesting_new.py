...
def get_data_precip_72jam(path_hujan_72_jam):
    ...
    # Step 3: Generate 72 hourly dates backward (including current time)
    hourly_dates = [current_time - timedelta(hours=i) for i in range(72)][::-1]
    ...
    for date in hourly_dates:
        ...
        if (str_date in stored_data) and (stored_data[str_date]['data name'] != "no data"):
            ...
            try:
                data_name, prec_value = get_data_from_biglake(date)
                data_list.append(prec_value)
                data_name_list.append(data_name)
                new_dict[str_date] = {'prec':prec_value, 'data name':data_name}
            except:
