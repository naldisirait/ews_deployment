...
class SimVP_RegModel(nn.Module):
    r"""SimVP Model
    ...
    def __init__(self, in_shape, hid_S=8, hid_T=8, N_S=2, N_T=2, model_type='gSTA',
                 mlp_ratio=8., drop=0.0, drop_path=0.0, spatio_kernel_enc=3,
                 spatio_kernel_dec=3, out_features=24, act_inplace=True, **kwargs):
        super(SimVP_RegModel, self).__init__()
        
        B, T, C, H, W = in_shape  # T is pre_seq_length
        H, W = int(H / 2**(N_S/2)), int(W / 2**(N_S/2))  # downsample 1 / 2**(N_S/2)
        act_inplace = False
        in_features = T*hid_T*2*2
        self.enc = Encoder(C, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.fc = FullyConnectedLayer(in_features, out_features)
        self.hid = MidMetaNet(T*hid_S, hid_T, N_T, input_resolution=(H, W), model_type=model_type,
...
class FullyConnectedLayer(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.layer2 = nn.Linear(256,out_features)
        self.silu = nn.SiLU()
    def forward(self, x):
